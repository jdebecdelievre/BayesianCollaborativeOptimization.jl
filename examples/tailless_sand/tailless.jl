...
function prepare_aero()
    ...
    return AIC, AIC_LU, TLmesh
end
@consts begin
    AIC, AIC_LU, TLmesh = prepare_aero()
    ...
    aero_local = (
        ...
        Sig      = Var(ini= ones(ny)*.1, lb=-5,  ub=5.),
        Sig_4g   = Var(ini= ones(ny)*.1, lb=-5,  ub=5.),
        dSig_dal = Var(ini= ones(ny)*.1, lb=-5,  ub=20.),
...
function aero(alpha, delta_e, twist, xcg, Sig, Sig_4g, dSig_dal)
    ...
    return D, load_4g, Cl, Cm, Cm_4g, q, Cm_al, Cm_al_4g, circCon, circCon_4g, circCon_dal
end
...
function solve_aao()
    ...
    function aao(g, x)
        ...
        # Aero
        D, load_4g, Cl, Cm, Cm_4g, q, Cm_al, Cm_al_4g, circCon, circCon_4g, circCon_dal = aero(x[idx.alpha], x[idx.delta_e], x[idx.twist], x[idx.xcg], x[idx.Sig], x[idx.Sig_4g], x[idx.dSig_dal])
        g[idg.qpos] = -q/W
        @. g[idg.Cl] = Cl / 1.45 - 1
        g[idg.Cm_al] = Cm_al
        ...
        g[idg.Cm] = Cm
        g[idg.Cm_4g] = Cm_4g
        g[idg.circCon] = circCon
        g[idg.circCon_4g] = circCon_4g
        g[idg.circCon_dal] = circCon_dal
    ...
    ug[idg.xcg] = 0.
    options = SNOW.Options(derivatives=SNOW.ForwardAD())
...
function BayesianCollaborativeOptimization.subspace(::Tailless, ::Val{:aero}, z::AbstractArray, filename::String, 
    ...
    if viol > 1e-6 
        println("aero constraint violation $filename: $viol")
    end
    return zs
end
...
function BayesianCollaborativeOptimization.subspace(::Tailless, ::Val{:struc}, z::AbstractArray, filename::String,
    ...
    viol = sum(max(0., lg[i]-g[i])+ max(0., g[i]-ug[i]) for i=1:Ng)
    if viol > 1e-6 
        println("struc constraint violation $filename: $viol")
    end
    # @assert (viol < 1e-6) "$viol"
    return zs
end
